#' Estimate the covariance using the PASS
PASScov = function(y, s) {
  ...
  Z.Mat = apply(Z.index, 2, function(Z.idx){
    j = Z.idx[1]; k = Z.idx[2]
    tempNorm = sqrt( sum( (y[j, ] - y[k, ])^2 ) * w)
    if (tempNorm == 0) {
      Zjk = matrix(rep(0, length(y[j, ])), nrow = 1)
    } else {
      Zjk = as.matrix((y[j, ] - y[k, ]) / tempNorm, nrow=1)
    }
    return(Zjk)
  })
...
#' Taken from https://github.com/kwgit/PASSrobustFPCA
#' Calculate the eigenvalues for the PASS covariance matrix
performPASSeigenAnalysis <- function (fdata,
  ...
  Q <- sum( !((cumsum(pass.evalues) / sum(pass.evalues)) >= 1) ) #  Wann erklären wir bereits alle Varianz mit den Komponenten?
  pass.evalue <- pass.evalues[1:Q]

  pass.ratio <- pass.evalue[-1]/pass.evalue[1]

  U.score <- getU(fdata, matrix(pass.eigen.fun[,1:Q]), trim = trim)
  ...
  return (list(efun = pass.eigen.fun, cpe = cpe, itr.ratio=itr.ratio))
}
